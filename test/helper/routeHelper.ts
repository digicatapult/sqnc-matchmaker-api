...
export const post = async (app: express.Express, endpoint: string, body: object, headers: object = {}): Promise<request.Test> => {
  return request(app).post(endpoint).send(body).set(headers)
}

export const postFile = async (app: express.Express, endpoint: string, buf: Buffer, filename: string): Promise<request.Test> => {
